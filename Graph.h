...
void Graph::addedge(const string& f, const string& t)
{
	vertex *from = (work.find(f)->second);
	vertex *to = (work.find(t)->second);
	vertex *edge = to;
	//pair<int, vertex *> edge = make_pair(cost, t);
	from->adj.push_back(edge);

}
void Graph::getCityEdge(City& f)
{
	vertex *from = (work.find(f.getCardName())->second);
	
	vector<vertex*>::iterator itr = from->adj.begin();
	while (itr != from->adj.end())
	{
		cout<< from->name;
		//string c = itr->first;
		//cout << "*"<<itr->first ;
